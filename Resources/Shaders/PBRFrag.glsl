...
vec3 Fr_Spec(float NdotV, float LdotH, float NdotH, float NdotL, vec3 f0, float roughness)
{
    ...
    //float V = V_GGX(NdotL, NdotV, roughness);
...
vec4 CalcLightInternal(BaseLight Light, vec3 LightDirection, vec3 WorldPos, vec3 N, vec3 diffuseAlbedo, vec3 f0)
{
    ...
    if (NdotL > 0.0)
    {
        ...
        vec3 spec = Fr_Spec(NdotV, LdotH, NdotH, NdotL, f0, gRoughness);
...
vec4 CalcDirectionalLight(vec3 WorldPos, vec3 Normal, vec3 diffuseAlbedo, vec3 f0)
{
    return CalcLightInternal(gDirectionalLight.Base, gDirectionalLight.Direction, WorldPos, Normal, diffuseAlbedo, f0);
}

vec4 CalcPointLight(int index, vec3 WorldPos, vec3 Normal, vec3 diffuseAlbedo, vec3 f0)
{
    vec3 LightDirection = WorldPos - gPointLight[index].Position;
    float Distance = length(LightDirection);
    
    vec4 color = CalcLightInternal(gPointLight[index].Base, LightDirection, WorldPos, Normal, diffuseAlbedo, f0);
...
void main()
{
    vec3 baseColor = texture(tex, Texcoord).xyz * Color;
   // baseColor = baseColor * vec3(1.0, 0.5, 0.5);

    
    vec3 diffuseColor = (1.0 - gMetalMask) * baseColor;
    
    float f0_term = 0.16 * gReflectivity * gReflectivity;
    vec3 f0 = vec3(f0_term, f0_term, f0_term);
    
    f0 = f0 * (1.0 - gMetalMask) + baseColor * gMetalMask;
    
    vec4 light = vec4(baseColor * vec3(0.0, 0.0, 0.0), 1.0);
    
    light += CalcDirectionalLight(WorldPos, Normal, diffuseColor, f0);
    for (int i = 0; i < gNumPointLights; i++)
    {
        light += CalcPointLight(i, WorldPos, Normal, diffuseColor, f0);
    }
